...
from elbepack.tests.test_xml import (  # noqa: F401
    base_image_build,
    simple_build,
    test_build_base_image,
    test_build_extended_image,
    test_check_updates,
    test_rebuild,
    test_simple_build,
)
...
_CONTAINERFILE_DIR = _REPO_ROOT / 'contrib' / 'containerfile-vmless'
...
def _run_build(elbe_buildenv_image, workdir, xml_name, build_args=(), base_image=None,
    ...
    if source_dir is None:
        ...
        mounts += ['-v', f'{source_dir.resolve()}:/src:z,ro']
        xml_container_path = f'/src/{xml_name}'
